...
import { ArrowLeft, CheckCircle2Icon, MessagesSquare, X } from "lucide-react"
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-[radial-gradient(circle_at_top,_rgba(226,232,240,0.9),_rgba(248,250,252,0.98)_42%,_rgba(255,255,255,1)_100%)]">
      <div className="min-h-full">
        <div className="border-b border-slate-200/80 bg-white/85 px-4 py-3 backdrop-blur lg:px-8 lg:py-5">
        ...
        <div className="px-4 py-4 lg:px-8 lg:py-6">
          <div className="mx-auto grid max-w-[1600px] gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
            <section className="flex flex-col rounded-[30px] border border-slate-200 bg-white/92 p-4 shadow-[0_28px_80px_rgba(15,23,42,0.12)] backdrop-blur lg:p-6 xl:p-8">
              <div className="grid gap-4 lg:gap-6">
                <div className="grid gap-2 rounded-2xl border border-slate-200 bg-slate-50/80 p-4 shadow-sm sm:gap-3 lg:p-5">
                  ...
                  <Input
                    ...
                    className="h-12 rounded-xl border-slate-200 bg-white px-4 text-sm shadow-none focus-visible:ring-1 focus-visible:ring-slate-300 lg:h-16 lg:text-lg"
                  />
                </div>

                <div className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50/70 p-4 shadow-sm sm:gap-4 lg:p-5">
            ...
            <aside className="flex flex-col rounded-[30px] border border-slate-200 bg-white/92 p-4 shadow-[0_24px_60px_rgba(15,23,42,0.1)] backdrop-blur lg:sticky lg:top-6 lg:self-start lg:p-5">
              ...
              <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div className="border-b border-slate-100 bg-slate-50/80 p-3 lg:p-4">
                  <Input
                    placeholder="Search for contact"
                    value={contactQuery}
                    onChange={(e) => setContactQuery(e.target.value)}
                    className="h-10 rounded-xl border-slate-200 bg-white text-sm shadow-none focus-visible:ring-1 focus-visible:ring-slate-300 lg:h-11"
                  />
                </div>
                <div className="max-h-[320px] overflow-y-auto divide-y divide-slate-100 pr-1 lg:max-h-[420px] lg:pr-2 scrollable-list">
                  {contactsList
                    .filter((contact) =>
                    ...
                    .map((contact) => {
                      ...
                      return (
                        <div
                          key={contact.email}
                          className="flex items-center gap-3 px-3 py-3 transition-colors hover:bg-slate-50 sm:px-4"
                        >
                          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 text-xs text-white shadow-sm sm:h-9 sm:w-9 sm:text-sm">
                            {initials}
                          </div>
              ...
              <div className="mt-4 flex flex-col gap-2 border-t border-slate-200 pt-4">
                <Button
                  className="h-11 w-full rounded-xl bg-slate-900 text-sm font-medium text-white hover:bg-slate-800 lg:h-12 lg:text-base"
                  type="button"
                  disabled={
                    isPending ||
                    displaySuccess ||
                ...
                <Button
                  type="button"
                  variant="outline"
                  className="h-11 w-full rounded-xl border-slate-200 text-sm text-slate-700 lg:h-12 lg:text-base"
                  onClick={() => onOpenChange(false)}
                >
                  Close
                </Button>
