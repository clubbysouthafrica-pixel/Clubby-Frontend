...
type TemplateVariableConfig =
  | string
  | (Partial<ClubVariable> & { title?: string });

const getTemplateVariableName = (variable: TemplateVariableConfig) =>
  typeof variable === "string" ? variable : variable.name ?? "";

const getTemplateVariableTitle = (variable: TemplateVariableConfig) =>
  typeof variable === "string"
    ? variable
    : variable.title ?? variable.name ?? "";

const getTemplateVariableKey = (variable: TemplateVariableConfig) =>
  typeof variable === "string"
    ? variable
    : variable.key ?? variable.name ?? "";

const getTemplateVariableRulesEngine = (variable: TemplateVariableConfig) =>
  typeof variable === "string" ? undefined : variable.rules_engine;
...
export default function PendingMembersList({
  ...
  const buildGeneratedTemplateVariables = useCallback((member: ClubMember) => {
    ...
    configuredVariables.forEach((rawVariable: TemplateVariableConfig) => {
      const variable: ClubVariable = {
        name: String(getTemplateVariableTitle(rawVariable)),
        key: String(getTemplateVariableKey(rawVariable)),
        visible: typeof rawVariable === "string" ? true : rawVariable.visible ?? true,
        rules_engine: getTemplateVariableRulesEngine(rawVariable),
      };
  ...
  const validateTemplateVariables = (member?: ClubMember): boolean => {
    ...
    const emptyFields = clubMembers.template_variables.filter(
      (variable: TemplateVariableConfig) => {
        const varName = getTemplateVariableName(variable);
        const varTitle = getTemplateVariableTitle(variable);
        const value = templateVariables[varName];
        const hasRulesEngine = isRulesEngineTemplateVariable(
          getTemplateVariableRulesEngine(variable),
        );
  ...
  const buildTemplateVariablesWithValues = (member: ClubMember) => {
    ...
    return clubMembers.template_variables.map((variable: TemplateVariableConfig) => {
      const varName = getTemplateVariableName(variable);
      const varTitle = getTemplateVariableTitle(variable);
      const autoGenerated = isRulesEngineTemplateVariable(
        getTemplateVariableRulesEngine(variable),
      );
      const value =
  ...
  const visibleTemplateVariables = useMemo(() => {
    if (!Array.isArray(clubMembers?.template_variables)) {
      return [];
    }

    return clubMembers.template_variables.filter(
      (variable: TemplateVariableConfig) => {
        const varName = getTemplateVariableName(variable);
        const varTitle = getTemplateVariableTitle(variable);

        return varName !== "member_name" && varTitle !== "Member Name";
      },
    );
  }, [clubMembers?.template_variables]);
                                        ...
                                        {isTemplateVariablesOpen && (
                                          <div className="mt-3 space-y-3 rounded-lg border border-slate-200 bg-white p-4">
                                            ...
                                            {[...visibleTemplateVariables]
                                              .sort((left: TemplateVariableConfig, right: TemplateVariableConfig) => {
                                                const leftAutoGenerated = isRulesEngineTemplateVariable(
                                                  getTemplateVariableRulesEngine(left),
                                                );
                                                const rightAutoGenerated = isRulesEngineTemplateVariable(
                                                  getTemplateVariableRulesEngine(right),
                                                );
